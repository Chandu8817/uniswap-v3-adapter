...
describe("UniswapV3Adapter", function () {
  ...
  describe("Deployment", function () {
    it("Should set the correct addresses", async function () {
      ...
      expect(await adapter.positionManager()).to.equal(
        ADDRESSES.NONFUNGIBLE_POSITION_MANAGER,
      );
  ...
  describe("addLiquidity", function () {
    it("Should add liquidity and emit LiquidityAdded event", async function () {
      const { adapter, usdc, weth, user } = await loadFixture(
        deployUniswapV3AdapterFixture,
      );
      ...
      const usdcApproveTx = await usdc
        .connect(user)
        .approve(await adapter.getAddress(), usdcAmount);
      await usdcApproveTx.wait();

      const wethApproveTx = await weth
        .connect(user)
        .approve(await adapter.getAddress(), wethAmount);
      ...
      const liquidityLog = receipt.logs.find(
        (log) =>
          log.address.toLowerCase() === String(adapter.target).toLowerCase(),
      ) as any;
  ...
  describe("withdrawLiquidity", function () {
    it("Should withdraw liquidity and emit LiquidityRemoved event", async function () {
      ...
      const liquidityLog = receiptAdd.logs.find(
        (log) =>
          log.address.toLowerCase() === String(adapter.target).toLowerCase(),
      ) as any;
      const tokenId = liquidityLog.args[0];
      ...
      const tx = await adapter
        .connect(user)
        .withdrawLiquidity(tokenId, liquidity, 0, 0, { from: user.address });
